...
//        Integer i=((Integer.valueOf("ffff",16).shortValue())/256-1);
//        System.out.println(i.floatValue()/10);
        //service.receiveMessage("s".getBytes());
        /*List<String> toUser=new ArrayList<>();
        toUser.add("aa");
        toUser.add("aa");
        toUser.add("aa");
        System.out.println(JSON.toJSONString(toUser));*/

        String s = "5a5ab02504100200ef04110203e8021702010302180203e8012002004305120400000b95d7";

        String[] strings = MessageUtil.handleStr2ArrStr(s);
        Integer[] integers = MessageUtil.handleStr2ArrInt(strings);

       for(int i = 0 ; i <strings.length; i++){
            System.out.println("string AND int:"+i+":"+strings[i] + ":"+integers[i]);
        }
    ...
    public void receiveMessage(byte[] payload) {    // 进行消息接收处理
        ...
        //String s = "5a5ab020041002015b04110202a8021702010d02180203e805120400007e5cdc";
        ...
        if("7b55".equals(s)){
            ...
            deviceGather.setDeviceSn("soilT");
            ...
            deviceGather.setDeviceSn("soilH");
            ...
            if(strings.length>32){
                ...
                deviceGather.setDeviceId(25);
                deviceGather.setDeviceName("电导率传感器");
                deviceGather.setDeviceSn("soilC");
                ...
                basicData = Integer.valueOf(strings[27] + strings[28] + strings[29] + strings[30], 16);
                deviceGather.setBasicData(basicData.floatValue());

                //获取当前设备的阈值,并进行阈值比较
                Device d7 = (Device) deviceThreshold.get(integers[25] + "");
                compareThreshold(jmsMessagingTemplate, d7, deviceGather.getBasicData());
                ...
                //电导率
                //获取当前设备的阈值,并进行阈值比较
                Device d5 = (Device) deviceThreshold.get("25");
                compareThreshold(jmsMessagingTemplate, d5, deviceGather.getBasicData());

                Integer ddl = 328;
                deviceGather.setBasicData(ddl.floatValue());
                deviceGather.setDeviceId(25);
                deviceGather.setDeviceName("电导率传感器");
                deviceGather.setDeviceSn("soilC");
                deviceGather.setDeviceType(integers[25] + "");
                ...
                deviceGather.setBasicData(basicData.floatValue() / 10);
                //获取当前设备的阈值,并进行阈值比较
                Device d6 = (Device) deviceThreshold.get("32");
                compareThreshold(jmsMessagingTemplate, d6, deviceGather.getBasicData());
                deviceGather.setDeviceId(32);
                deviceGather.setDeviceName("PH值传感器");
                deviceGather.setDeviceSn("dPH");
                deviceGather.setDeviceType("32");
                deviceGather.setMeasurementUnitId(6);
                deviceGather.setMeasurementUnitName("PH");
                deviceGather.setMeasureUnitType("PH值");
                deviceGatherDao.save(deviceGather);
