...
public class TTaskController {
    ...
    public RespEntiry pushTask(@RequestBody Map<String, String> map) {
        try {
            ...
            //出库
            TTask task =  tTaskDao.getById(Long.parseLong(id));
            //查询是否有出库
            if(!StringUtils.isEmpty(task.getInputsBatchesId())){
                String inputsNumber = map.get("inputsNumber"); //获取数量
                if(isNumericZidai(inputsNumber)){
                    inputsNumber = map.get("inputsNumber");

                    //修改批次数量
                    String batchId = task.getInputsBatchesId();
                    InputsBatch inputsBatch = inputsBatchDao.getById(batchId);
                    Double stockNumber =  Double.parseDouble(inputsBatch.getStockNumber());
                    Double number =  Double.parseDouble(inputsNumber);
                    Double snumber;
                ...
                //添加出库记录
                InputsOut inputsOut = new InputsOut();
                inputsOut.setBatchId(task.getInputsBatchesId());
                inputsOut.setOutStockNumber(inputsNumber);
                inputsOut.setUsername(id);
                inputsOut.setRemarks(area);
                inputsOut.setOutStockDate(new Date());
                inputsOut.setMakeArea(task.getMakeArea());
                inputsOut.setMakeMethod(task.getMakeMethod());
                inputsOut.setMakeMassif(task.getMakeMassif());
                inputsOutDao.save(inputsOut);
            }
    ...
    public RespEntiry pullTask(@RequestBody Map<String, String> map) {
        try {
            ...
            String makeArea = map.get("makeArea");
            ...
            task.setMakeArea(makeArea);

            String batchId = map.get("batchId");
            String inputsNumber = "";

            if(!StringUtils.isEmpty(batchId) && !batchId.equals("1")){
                inputsNumber = map.get("inputsNumber");
                task.setInputsBatchesId(batchId);
                task.setInputsActNumber(inputsNumber);
                task.setMakeMethod(map.get("makeMethod"));
                task.setMakeMassif(map.get("makeMassif"));
                tTaskDao.updatePull2(task);
            }else{
    ...
    RespEntiry producterIdList(Integer producter_id, Integer state, @RequestParam(value = "p", defaultValue = "1") int p, @RequestParam(value = "psize", defaultValue = "5") int psize) {
        ...
        for(TTask task : producterIdList){
            List<InputsAct> list =  inputsActDao.getByBatchIdInfo(task.getInputsBatchesId());
            if(list.size()>0){
                task.setInputsAct(list.get(0));
            }else{
                InputsAct inputsAct = new InputsAct();
                inputsAct.setInputsName("无");
                task.setInputsAct(inputsAct);
            }
        }
    ...
    RespEntiry mobile(String id, String farmid) {
        try {
            ...
            List<Vedio> listVedio = vedioDao.getVedioListByFarmId(Long.parseLong(farmid));
            ...
            taskMobileDto.setListVedio(listVedio);
    ...
    private JSONObject sendMessage(TTask tTask, TProducter producter) {
        ...
        String token = "26_ZZDrU57jCBcuOqUG88Yjga8NrXsdTLmW1LHzejVCNDoyPbHjfTeSgizr1lEXvWcoum9CiYZ1fvNKCopiZGJ4QT0OOcnIHQC61C1B4l_fPJCsC0L2F9mKhzjCMqEVRHaAHACYO";
        return SendWxTemplateMessageUtil.sendMessage(map, producter.getOpenid(), WXConstants.TASK_MESSAGE_ID, token);
    }
